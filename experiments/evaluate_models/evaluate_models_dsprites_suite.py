"""
CEM/CRCEM evaluation suite for dSprites (dep=0, 5 concepts).
...
Results saved to  results/results_dsprites_suite.dict
Plots saved to    results/plots/dsprites/
"""
...
from experiments.evaluate_models.eval_suite import (
    ...
    task_leakage_probe,
    dsprites_error_set_analysis, print_dsprites_leakage,
...
BASELINE_CACHE  = master_folder + "results/cache/dsprites_icl_baseline.joblib"
    ...
    for label, fold_ckpts in [("cem", cem_ckpts), ("crcem", crcem_ckpts)]:
        ...
        for fold, ckpt in fold_ckpts.items():
            ...
            todo = {
                ...
                "ds_leak":   "ds_leak"         not in r or RERUN_DS_LEAK,
            ...
            # --- dSprites hard leakage test ---
            if todo["ds_leak"]:
                r["ds_leak"] = dsprites_error_set_analysis(
                    test_preds["c_pred"], test_preds["c_true"],
                    test_preds["y_pred"], test_preds["y_true"],
                )
            print_dsprites_leakage(r["ds_leak"], label=f"{label} {fold}")
...
_vis_crcem_ckpts = find_checkpoints(CRCEM_FOLDER, "CRCEM_", _vis_lam, FOLDS)

print("\nSaving PCA scatter plots...")
for label, fold_ckpts in [("cem", _vis_cem_ckpts), ("crcem", _vis_crcem_ckpts)]:
...
N_CONCEPTS  = cem_probe.shape[0]
for k in range(N_CONCEPTS):
    plot_probe_curve(
        cem_probe[k], crcem_probe[k],
        title=f"dSprites — concept {k} probe accuracy ({first_fold}, {_vis_lam})",
        save_path=PLOT_DIR + f"probe_curve_concept{k}.png",
        average=False,
    )
plot_probe_curve(
    cem_probe, crcem_probe,
    title=f"dSprites — probe accuracy avg over concepts ({first_fold}, {_vis_lam})",
    save_path=PLOT_DIR + "probe_curve_avg.png",
    average=True,
)

# ---------------------------------------------------------------------------
# Prototype alignment plot — fold_1, lam_c0.1, CEM vs CRCEM side-by-side
# ---------------------------------------------------------------------------
print("\nSaving prototype alignment plot...")
first_fold  = next(iter(_vis_cem_ckpts))
cem_preds   = load_predictions(_vis_cem_ckpts[first_fold],   x2c_extractor, test_dl)
crcem_preds = load_predictions(_vis_crcem_ckpts[first_fold], x2c_extractor, test_dl)
proto_scores = prototype_alignment_plot(
    ...
    title=f"dSprites — prototype alignment ({first_fold}, {_vis_lam})",
...
print(f"  CRCEM pos-neg dist={proto_scores['CRCEM']['pos_neg_dist']:.4f}"
      f"  concept sep={proto_scores['CRCEM']['concept_sep']:.4f}")
...
for label in ["cem", "crcem"]:
    ...
    for lam_c in LAM_C_LIST:
        ...
        for fold, r in results[label][lam_c].items():
            K = len(r.get("ctl_mean", [0]))
            mask = ~np.eye(K, dtype=bool) if K > 0 else None
            ...
            ctl_str   = f"  CTL={np.mean(r['ctl_mean']):.4f}" if "ctl_mean" in r else ""
            icl_str   = f"  ICL={r['icl_mean']:.4f}" if "icl_mean" in r else ""
            cvl_str   = f"  CVL={r['cvl']['CVL']:.4f}" if "cvl" in r else ""
            icvl_str  = f"  ICVL={r['icvl']['ICVL']:.4f}" if "icvl" in r else ""
            iclp_str  = ""
            if "icl_probe" in r and mask is not None:
                iclp_str = f"  ICL_Δacc={r['icl_probe']['acc_diff'][mask].mean():.4f}"
            onc_str = ""
            if "ois_nis_cas" in r:
                onc = r["ois_nis_cas"]
                onc_str = f"  OIS={onc['ois']:.4f}  NIS={onc['nis']:.4f}  CAS={onc['cas']:.4f}"
            print(f"    {fold}  task={r['task_acc']:.4f}{ctl_str}{icl_str}"
                  f"{cvl_str}{cvl_g_str}{icvl_str}"
                  f"  err_acc={acc_wrong:.4f}/{n_wrong}"
                  f"{iclp_str}{epd_str}{onc_str}")
