"""CEM intervention plot — dSprites."""
import os
import numpy as np
...
for lam, color, llab in zip(LAMS, C_GCCEM, LAM_LABEL):
    plot_curve(ax, get_curves(d, "crcem", lam), color, label=f"GC-CEM {llab}")
...
ax.legend(fontsize=8.5, loc="center left", ncol=2, framealpha=0.9)

plt.tight_layout()
for ext in [".pdf", ".png"]:
    out = master + f"experiments/evaluate_models/intervention_cem_dsprites{ext}"
