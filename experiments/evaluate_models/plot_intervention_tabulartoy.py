"""
Intervention plot for TabularToy — CBM family only.
...
Run from project root:
    python experiments/evaluate_models/plot_intervention_tabulartoy.py
"""
import os
import numpy as np
...
# Load data
# ---------------------------------------------------------------------------
cbm_d = joblib.load(master_folder + "results/results_tabulartoy_cbm_suite.dict")


def get_curves(suite, model_key, lam_key):
    """Return (n_folds, n_steps) array of intervention curves in percent."""
    curves = []
...
# Style
# ---------------------------------------------------------------------------
STEPS = np.array([0, 1, 2, 3])   # concepts intervened (out of 3)

LAMS      = ["lam_c0.1", "lam_c0.5", "lam_c1"]
LAM_LABEL = [r"$\lambda_c=0.1$", r"$\lambda_c=0.5$", r"$\lambda_c=1.0$"]

# Distinct, accessible colours
C_HARD  = "#2c2c2c"       # near-black
C_SEQ   = "#7f7f7f"       # mid-grey
C_JOINT = ["#f4a896", "#e05c3a", "#8b1a00"]   # light→dark red
C_GCCBM = ["#92c4de", "#2b7ab8", "#003f7d"]   # light→dark blue
...
for lam, color, llab in zip(LAMS, C_GCCBM, LAM_LABEL):
    plot_curve(ax, get_curves(cbm_d, "acbm", lam),
...
plt.tight_layout()
out     = master_folder + "experiments/evaluate_models/intervention_tabulartoy.pdf"
