"""
Intervention plot for dSprites — CBM family only.
...
Run from project root:
    python experiments/evaluate_models/plot_intervention_dsprites.py
"""
import os
import numpy as np
...
# Load data
# ---------------------------------------------------------------------------
cbm_d = joblib.load(master_folder + "results/results_dsprites_cbm_suite.dict")
...
# ---------------------------------------------------------------------------
n_concepts = 5
STEPS = np.arange(n_concepts + 1)   # 0 … 5 concepts intervened
...
ax.set_xticks(STEPS)
ax.set_xlim(-0.1, n_concepts + 0.1)
ax.set_ylim(93, 101)
ax.grid(True, alpha=0.3, linewidth=0.5)
ax.legend(fontsize=8.5, loc="lower right", ncol=2, framealpha=0.9)

plt.tight_layout()
out     = master_folder + "experiments/evaluate_models/intervention_dsprites.pdf"
